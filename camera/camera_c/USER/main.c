...
volatile u8 jpeg_data_ok
//作者：正点原子 @ALIENTEK
//作者：正点原子 @ALIENTEK=0;				//JPEG数据采集完成标志 
										//0,数据没有采集完;
										//1,数据采集完了,但是还没处理;
...
//作者：正点原子 @ALIENTEK		u8 msgbuf[15];	//消息缓存区 
	L
//作者：正点原子 @ALIENTEKCD_Clear(WHITE);
    P
//作者：正点原子 @ALIENTEKOINT_COLOR=RED; 
	L
//作者：正点原子 @ALIENTEKCD_ShowString(30,50,200,16,16,"ALIENTEK STM32F4");
...
//作者：正点原子 @ALIENTEK
	...
	LCD_ShowString(30,100,200,16,16,"KEY0:Contrast");			//对比度
	LCD_ShowString(30,130,
//作者：正点原子 @ALIENTEK200,16,16,"KEY1:Saturation"); 		//色彩饱和度
	LCD_ShowString(30,150,
//作者：正点原子 @ALIENTEK200,16,16,"KEY2:Effects"); 			//特效 
	LCD_ShowString(30,170,
//作者：正点原子 @ALIENTEK200,16,16,"KEY_UP:FullSize/Scale");	//1:1尺寸(显示真实尺寸)/全尺寸缩放
		}
//作者：正点原子 @ALIENTEK
	}
//作者：正点原子 @ALIENTEK
...
//作者：正点原子 @ALIENTEK
		...
		{  
			p=(u8*)jpeg_buf;
			LCD_ShowString(30,210,210,16,16,"	u8 msgbuf[15];	//消息缓存区 
	LCD_Clear(WHITE);
    POINT_COLOR=RED; 
	LCD_ShowString(30,50,200,16,16,"ALIENTEK STM32F4");
	LCD_ShowString(30,70,200,16,16,"OV2640 RGB565 Mode");
	...
	LCD_ShowString(30,150,200,16,16,"KEY2:Effects"); 			//特效 
	LCD_ShowString(30,170,200,16,16,"KEY_UP:FullSize/Scale");	//1:1尺寸(显示真实尺寸)/全尺寸缩放
			for(i=0;i<jpeg_data_len*4;i++)		u8 msgbuf[15];	//消息缓存区 
	LCD_Clear(WHITE);
    POINT_COLOR=RED; 
	LCD_ShowString(30,50,200,16,16,"ALIENTEK STM32F4");
	LCD_ShowString(30,70,200,16,16,"OV2640 RGB565 Mode");
	...
	LCD_ShowString(30,170,200,16,16,"KEY_UP:FullSize/Scale");	//1:1尺寸(显示真实尺寸)/全尺寸缩放
			{
        while(USART_GetFlagStatus(USART2,USAR	u8 msgbuf[15];	//消息缓存区 
	LCD_Clear(WHITE);
...
//作者：正点原子 @ALIENTEK
	...
	LCD_ShowString(30,150,200,16,16,"KEY2:Effects"); 			//特效 
	LCD_ShowString(30,170,200,16,16,"KEY_UP:FullSize/Scale");	//1:1尺寸(显示真实尺寸)/全尺寸缩放
				USART_SendData(USART2,p[i]); 	u8 msgbuf[15];	//消息缓存区 
	LCD_Clear(WHITE);
    POINT_COLOR=RED; 
	LCD_ShowString(30,50,200,16,16,"ALIENTEK STM32F4");
	LCD_ShowString(30,70,200,16,16,"OV2640 RGB565 Mode");
	...
	LCD_ShowString(30,150,200,16,16,"KEY2:Effects"); 			//特效 
	LCD_ShowString(30,170,200,16,16,"KEY_UP:FullSize/Scale");	//1:1尺寸(显示真实尺寸)/全尺寸缩放
					case KEY0_PRES:	u8 msgbuf[15];	//消息缓存区 
	LCD_Clear(WHITE);
    POINT_COLOR=RED; 
	LCD_ShowString(30,50,200,16,16,"ALIENTEK STM32F4");
	LCD_ShowString(30,70,200,16,16,"OV2640 RGB565 Mode");
